...
def send_api():
    ...
    【文章の修正】
    ・入力された文章をそのまま使用するのではなく、意味を変えない範囲で自然なビジネスメールに書き直す
    ・単純に文末だけを敬語に変えるのではなく、社会人が企業や目上の相手に送るメールとして自然な文章構成にする
    ・入力内容に含まれる感謝、感想、依頼、報告などの意図を維持したまま、適切な敬語表現にする
    ・入力文がすでに敬語であっても、より自然なビジネスメールになる場合は表現を改善する
    ・必要に応じて分奏の順番や表現を整理する
    ・「です・ます」にするだけで終わらせず、文脈に合った敬語表現にする
    ・不自然な二重敬語を使用しない
    ・相手に失礼な表現を使用しない
    ・不自然な直訳調の日本語を使用しない
    ・同じ意味の文章を繰り返さない
    ・ただし、入力されていない事実、人物名、会社名、日付、予定などは追加しない

    【敬語への変換例】
    ・「インターンシップありがとうございました。」
        →「インターンシップでは、大変お世話になり、ありがとうございました。」

    ・「社員の方々が親切でした。」
        →「社員の皆様が親切に接してください、働きやすい環境だと感じました。」

    ・「修了証明書と評価書を添付します。」
        →「修了証明書と評価書を添付いたします。」

    ・「この経験を活かします。」
        →「今回の経験を今後に生かしたいと考えております。」

    ・「とてもいい経験になりました。」
        →「今回のインターンシップを通して、とてもいい経験をすることができました。」
...
現在のメール内容をもとに、追加指示を反映してメールを再生成してください。
元の入力内容の意味や事実は変更しないでください。
追加指示にない情報を勝手に追加しないでください。
"""

...
def send_api():
    ...
    ・「［あなたの名前］」などの仮の情報を勝手に追加しない
    ・入力されていない出来事を勝手に追加しない
    ・入力内容の意味を変えない
    ・文章を読みやすき整理する
    ・本文の冒頭に自己紹介を追加しない
    ・「こんにちは」「〇〇大学の〇〇です」などの自己紹介は、ユーザーが明示的に入力した場合のみ使用する
    ・名前が入力されていない場合、名前を含む自己紹介を作成しない
    ・です・ます調で統一する
    ・入力文の内容は維持するが、単純に文末だけを敬語に変えるのではなく、
    日本語として自然なビジネスメールになるように文章全体を整理する
    ・ただし、入力されていない情報は追加しない
    ・「ありがとうございました」だけではなく、ビジネスメールとして自然な場合は
    「大変お世話になり、ありがとうございました」などの表現にする
    ・「終了証明書と評価書を添付します」は、
    「終了証明書と評価書を添付いたします」など、相手に送るメールとして適切な敬語にする
    ・不自然な二重敬語を使用しない
    ・相手に失礼な表現を使用しない
    ・不自然な直訳調の日本語を使用しない
    ・同じ意味の文章を繰り返さない
    ・件名はメールの内容がわかる簡潔なものにする
    ・本文には自然な挨拶、本文、結びを使用する
    ・入力内容にない出来事を定型文として追加しない
    ・送信相手との関係に適した敬語を使用する
    ・読みやすく自然な日本語にする
    ・元の内容を変えない
    ・指定された文章の雰囲気を反映する
